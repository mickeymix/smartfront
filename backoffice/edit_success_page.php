...
if ($_POST["action"] == "1") {
    ...
    if($uploadfile <>'' && $uploadfileemail_image_title<>''){
        ...
        ,email_success_link = '$email_success_link' WHERE email_menu_id = '2'";
   
    }else if ($uploadfile <>''){
        $sql = "UPDATE email_menu_config_master SET 
        email_success_dialog_image = '$target_file' WHERE email_menu_id = '2'";
    }else if ($uploadfileemail_image_title <>''){
        $sql = "UPDATE email_menu_config_master SET 
        email_image_title = '$target_fileTitle' WHERE email_menu_id = '2'";
    }else{
        $sql = "UPDATE email_menu_config_master SET email_success_link = '$email_success_link' WHERE email_menu_id = '2'";
    }
    ...
    <form action="edit_success_page.php" method="POST" class="form-horizontal" enctype="multipart/form-data">
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h2>ตั้งค่า Success Page แคตตาล๊อค</h2>
                    </div>
                </div>
                <!-- /. ROW  -->
                <hr />
                ...
                <?
                $sql = "SELECT * FROM email_menu_config_master where email_menu_id = '2'";
                // console.log($_GET["id"]);
