...
if ($_POST["action"] == "1") {
    ...
    if ($uploadfile <> '' && $uploadfileemail_button_image <> '' && $uploadfileemail_image_left <> '' && $uploadfileemail_image_right <> '') {
        ...
        , email_image_right = '$target_imageright' WHERE email_menu_id = '3'";
    } else 
    if ($uploadfile <> '') {
        $sql = "UPDATE email_menu_config_master SET email_image_title = '$target_file'
        , email_email_place = '$email_email_place'
        , email_name_place_holder = '$email_name_place_holder'
        WHERE email_menu_id = '3'";
    } else if ($uploadfileemail_button_image <> '') {
        $sql = "UPDATE email_menu_config_master SET
         email_email_place = '$email_email_place'
        , email_name_place_holder = '$email_name_place_holder'
        , email_button_image = '$target_fileButton'  WHERE email_menu_id = '3'";
    } else if ($uploadfileemail_image_left <> '') {
        $sql = "UPDATE email_menu_config_master SET 
         email_email_place = '$email_email_place'
        , email_name_place_holder = '$email_name_place_holder'
        , email_image_left = '$target_imageleft' WHERE email_menu_id = '3'";
    }
        ...
        $sql = "UPDATE email_menu_config_master SET 
         email_email_place = '$email_email_place'
        , email_name_place_holder = '$email_name_place_holder' WHERE email_menu_id = '3'";
    }
...
<div id="wrapper">
    ...
    <form action="edit_menu_image_click.php" method="POST" class="form-horizontal" enctype="multipart/form-data">
        <div id="page-wrapper">
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        <h2>ตั้งค่า dialog Whitepaper</h2>
                    </div>
                </div>
                <!-- /. ROW  -->
                <hr />
